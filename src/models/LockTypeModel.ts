...
export const create = async (lockType: LockType) => {

    const queryText = 'INSERT INTO lock_type(\
    name, \
    description) \
    ...
    const res: LockType = recursiveToCamel(rows[0])
    return res;

}

export const getById = async (id: number) => {
    const {rows} = await query('SELECT * FROM lock_type WHERE id=$1', [id])

    if (!rows[0]) {
        return undefined
    }

    const lockType: LockType = recursiveToCamel(rows[0]);
    return lockType
}
...
export const deleteById = async (id: number) => {
    await query('DELETE FROM lock_type WHERE id=$1', [id]);
    return
}
...
export const updateById = async (id: number, newProps: any) => {
    ...
    const queryText = `UPDATE lock_type
                       SET ${querys.join(',')}
...
export const search = async (props: any, page: number = 1) => {
    ...
    return [rows.map(x => recursiveToCamel(x) as LockType), rows[0] ? rows[0].count : 0];
}

const count = async () => {

    const {rows} = await query('SELECT COUNT(*) FROM lock_type', []);
