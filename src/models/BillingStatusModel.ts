...
export const create = async (billingStatus: BillingStatus) => {
  const queryText =
  ...
  const res: BillingStatus = recursiveToCamel(rows[0]);
  return res;
};

export const getById = async (id: number) => {
  const { rows } = await query("SELECT * FROM billing_status WHERE id=$1", [
    id,
  ]);
  ...
  const billingStatus: BillingStatus = recursiveToCamel(rows[0]);
  return billingStatus;
};

export const getAll = async () => {
  const { rows } = await query("SELECT * FROM billing_status", []);
  return rows.map((x) => recursiveToCamel(x) as BillingStatus);
};
...
export const deleteById = async (id: number) => {
  await query("DELETE FROM billing_status WHERE id=$1", [id]);
  return;
};
...
export const updateById = async (id: number, newProps: any) => {
  ...
  const queryText = `UPDATE billing_status
                       SET ${querys.join(",")}
                       WHERE id = $1 RETURNING *`;

  const { rows } = await query(queryText, [id, ...values]);

  const billing_status: BillingStatus = recursiveToCamel(rows[0]);
...
export const search = async (props: any, page: number = 1) => {
  ...
  return [
    rows.map((x) => recursiveToCamel(x) as BillingStatus),
    rows[0] ? rows[0].count : 0,
  ];
};

const count = async () => {
  const { rows } = await query("SELECT COUNT(*) FROM billing_status", []);
