...
export const create = async (deviceBreakIn: DeviceBreakIn) => {
  const queryText =
    "INSERT INTO device_break_in(\
    week_number, \
    description \
    ) \
    VALUES ($1, $2) RETURNING *";

  console.log(deviceBreakIn);
  console.log(deviceBreakIn.description);
  console.log(deviceBreakIn.weekNumber);

  const { rows } = await query(queryText, [
    deviceBreakIn.weekNumber,
    deviceBreakIn.description,
  ]);

  const res: DeviceBreakIn = recursiveToCamel(rows[0]);

  return res;
};

export const getById = async (id: number) => {
  const { rows } = await query("SELECT * FROM device_break_in WHERE id=$1", [
    id,
  ]);
  ...
  const deviceBreakIn: DeviceBreakIn = recursiveToCamel(rows[0]);
  return deviceBreakIn;
};
...
export const deleteById = async (id: number) => {
  await query("DELETE FROM device_break_in WHERE id=$1", [id]);
  return;
};
...
export const updateById = async (id: number, newProps: any) => {
  ...
  const queryText = `UPDATE device_break_in
                       SET ${querys.join(",")}
...
export const search = async (props: any, page: number = 1) => {
  ...
  let queryText = `
    SELECT device_break_in.*, 
           count(*) OVER () AS count
    FROM device_break_in
    ${querys.length ? "WHERE " + querys.join(" AND ") : ""}
    ORDER BY device_break_in.id ASC
    LIMIT 10
    OFFSET (($${i} - 1) * 10)
  `;
  ...
  return [
    rows.map((x) => recursiveToCamel(x) as DeviceBreakIn),
    rows[0] ? rows[0].count : 0,
  ];
};

const count = async () => {
  const { rows } = await query("SELECT COUNT(*) FROM device_break_in", []);
...
export default DeviceBreakInModal;
