...
export const create = async (healthCheckin: HealthCheckin) => {

    const queryText = 'INSERT INTO health_checkin(\
    name, \
    description) \
    ...
    const {rows} = await query(queryText, [
        healthCheckin.name,
        healthCheckin.description,
    ])

    const res: HealthCheckin = recursiveToCamel(rows[0])
    return res;

}

export const getById = async (id: number) => {
    const {rows} = await query('SELECT * FROM health_checkin WHERE id=$1', [id])

    if (!rows[0]) {
        return undefined
    }

    const healthCheckin: HealthCheckin = recursiveToCamel(rows[0]);
...
export const deleteById = async (id: number) => {
    await query('DELETE FROM health_checkin WHERE id=$1', [id]);
    return
}
...
export const updateById = async (id: number, newProps: any) => {
    ...
    const queryText = `UPDATE health_checkin
                       SET ${querys.join(',')}
...
export const search = async (props: any, page: number = 1) => {
    ...
    return [rows.map(x => recursiveToCamel(x) as HealthCheckin), rows[0] ? rows[0].count : 0];
}

const count = async () => {

    const {rows} = await query('SELECT COUNT(*) FROM health_checkin', []);
