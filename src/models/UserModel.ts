...
export const getTotalActiveUsers = async ()=> {
    const {rows} = await query(`SELECT account_status, COUNT(*) as user_count
                                FROM users
...
export const search = async (props: any, page: number = 1, orderBy?: string, orderDirection?: string) => {
    const querys: string[] = [];
    const values: any[] = [];
    ...
    let queryText = `SELECT *, count(*) OVER () AS count
                     ...
                     ORDER BY ${orderBy ?? "id"} ${orderDirection ?? "asc"}
                     LIMIT 10
                     OFFSET (($${i} - 1) * 10)`;
