...
export const create = async (gameStatus: DeviceType) => {

    const queryText = 'INSERT INTO game_status (\
    name) \
    VALUES ($1) RETURNING *'
    ...
    const res: GameStatus = recursiveToCamel(rows[0])
    return res;

}

export const getById = async (id: number) => {
    const {rows} = await query('SELECT * FROM game_status WHERE id=$1', [id])

    if (!rows[0]) {
        return undefined
    }

    const game_status: GameStatus = recursiveToCamel(rows[0]);
    return game_status
}

export const deleteById = async (id: number) => {
    await query('DELETE FROM game_status WHERE id=$1', [id]);
    return
}
...
export const updateById = async (id: number, newProps: any) => {
    ...
    const queryText = `UPDATE game_status
                       SET ${querys.join(',')}
                       WHERE id = $1 RETURNING *`

    const {rows} = await query(queryText, [id, ...values]);

    const gameStatus: GameStatus = recursiveToCamel(rows[0])
    return gameStatus;
}
